<?php
/**
 * OCOSIS — Ocorrências Pendentes
 * Integrado com os arquivos compartilhados do projeto:
 *   - pdo.php  -> conexão única com o banco (fica na mesma pasta)
 *
 * Obs.: assim como no perfil.php, o login desse sistema guarda o estado
 * em sessionStorage (JS), não em $_SESSION do PHP. Por isso a checagem
 * de "logado ou não" é feita no <script> logo no início do <body>.
 */

require_once __DIR__ . '/pdo.php';

/* ════════════════════════════════════════════════════════════════
   AJAX — Atualizar status de uma ocorrência (pendente/resolvida)
   ════════════════════════════════════════════════════════════════ */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['ajax_action'] ?? '') === 'atualizar_status') {
    ...
    try {
        ...
        if ($stmt->rowCount() === 0) {
            // Não é necessariamente erro (status pode já estar igual), mas
            // avisamos se o id simplesmente não existe no banco.
            $check = $pdo->prepare("SELECT 1 FROM ocorrencias WHERE id_ocorrencia = :id");
            $check->execute(['id' => $id]);
            if (!$check->fetchColumn()) {
                throw new Exception('Ocorrência não encontrada no banco de dados.');
            }
        }
...
// Todas as ocorrências pendentes, com dados de aluno/turma/disciplina/professor
// e as infrações agregadas (uma ocorrência pode ter mais de uma infração).
$sqlPendentes = $pdo->query("
    SELECT
        o.id_ocorrencia          AS id,
        o.id_aluno                AS id_aluno,
        a.nome_aluno               AS aluno,
        t.desc_turma               AS turma,
        o.data_ocorrencia          AS data_ocorrencia,
        o.horario                  AS horario,
        o.desc_ocorrencia          AS obs,
        o.notificar_responsavel    AS notif_responsavel,
        d.desc_disciplina          AS disciplina,
        fu.nome_funcionario        AS professor,
        GROUP_CONCAT(tp.num_item ORDER BY tp.num_item SEPARATOR ',')      AS infracoes_ids,
        GROUP_CONCAT(tp.desc_ocorrencia ORDER BY tp.num_item SEPARATOR '; ') AS infracoes_texto
    FROM ocorrencias o
    JOIN alunos a               ON a.id_aluno = o.id_aluno
    LEFT JOIN turma t           ON t.id_turma = o.id_turma
    ...
    LEFT JOIN tipo_ocorrencia tp  ON tp.id_tipo_ocorrencia = ot.id_tipo_ocorrencia
    WHERE o.status = 'pendente'
    GROUP BY o.id_ocorrencia
    ORDER BY o.data_ocorrencia DESC, o.horario DESC
")->fetchAll(PDO::FETCH_ASSOC);

$totalPendentes = count($sqlPendentes);

// Lista única de alunos com pendências, na ordem em que aparecem (para os chips)
$alunosPendentes = [];
foreach ($sqlPendentes as $row) {
    $aid = $row['id_aluno'];
    if (!isset($alunosPendentes[$aid])) {
        $alunosPendentes[$aid] = $row['aluno'];
    }
}

// Lista oficial de infrações (pra folha de impressão) — vem do banco,
// então fica sempre igual ao que está cadastrado em tipo_ocorrencia.
$tiposInfracaoImpressao = $pdo->query("SELECT num_item, desc_ocorrencia FROM tipo_ocorrencia ORDER BY num_item")
...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .empty-state .empty-icon { font-size: 2.2rem; display: block; margin-bottom: 0.75rem; }
...
document.addEventListener("DOMContentLoaded", function() {
    // Mesma checagem usada no perfil.php: o login guarda o estado no
    // sessionStorage do navegador, não em $_SESSION do PHP.
...
<main class="main">
    <div class="page-header">
        <h1 class="page-title">
            Ocorrências Pendentes
            <span class="badge-pendentes"><?= $totalPendentes ?> pendente<?= $totalPendentes === 1 ? '' : 's' ?></span>
        </h1>
    </div>
    ...
    <?php if ($totalPendentes > 0): ?>
        <div class="alert-pendentes">
            ...
            <div class="chips-alunos">
                <?php foreach ($alunosPendentes as $aid => $nome): ?>
                    <a href="perfil.php?id=<?= (int)$aid ?>" class="chip-aluno" style="text-decoration:none;">
        ...
        <div class="table-card">
            <table class="ocorrencias-table" id="tabela-pendentes">
                ...
                <tbody id="tabela-pendentes-corpo">
                    <?php foreach ($sqlPendentes as $oc):
                        $dataFormatada = date('d/m/Y', strtotime($oc['data_ocorrencia']));
                        $horaFormatada = substr($oc['horario'], 0, 5);
                        $materiaProfessor = trim(($oc['disciplina'] ?? '—') . ' / ' . ($oc['professor'] ?? '—'), ' /');
                        if ($materiaProfessor === '') { $materiaProfessor = '—'; }
                        $infracoesTexto = $oc['infracoes_texto'] !== null ? str_replace(';', ';', $oc['infracoes_texto']) : ($oc['obs'] ?: '—');
                    ...
                    ?>
                        <tr data-occ-id="<?= $oc['id'] ?>" data-aluno-id="<?= $oc['id_aluno'] ?>" class="linha-destacada">
                            <td><?= $dataFormatada ?></td>
                            <td>
                            ...
                            <td class="td-infracoes"><?= htmlspecialchars($infracoesTexto) ?></td>
                            <td class="td-materia"><?= htmlspecialchars($materiaProfessor) ?></td>
                            <td class="td-status">
                                <span class="status-pill status-pendente">
                                    Pendente
                                </span>
                                <?php if ((int)$oc['notif_responsavel'] === 1): ?>
                            ...
                            <td>
                                <div class="td-acoes">
                                    <button type="button" class="btn-editar"
                                        data-occ="<?= htmlspecialchars(json_encode($dadosEdicao), ENT_QUOTES, 'UTF-8') ?>">
                                        Atualizar Status
                                    </button>

                                    <a href="perfil.php?id=<?= (int)$oc['id_aluno'] ?>" class="btn-perfil">Ver Perfil</a>
                                    ...
                                    <button type="button" class="btn-imprimir" title="Imprimir Ocorrência"
                                        ...
                                        data-print-data="<?= htmlspecialchars($dataFormatada) ?>"
                                        data-print-horario="<?= htmlspecialchars($horaFormatada . ' · ' . ($oc['disciplina'] ?? '')) ?>"
                                        data-print-infracoes="<?= htmlspecialchars($oc['infracoes_ids'] ?? '') ?>"
                                        data-print-obs="<?= htmlspecialchars($oc['obs'] ?? '') ?>">
...
<div class="modal-overlay" id="modal-overlay" hidden>
    <div class="modal-editar" role="dialog" aria-modal="true" aria-labelledby="modal-titulo">
        <div class="modal-header">
            ...
            <button type="button" class="modal-fechar" id="modal-fechar" aria-label="Fechar">×</button>
        </div>
...
<div id="folha-impressao-container">
    ...
    <ul class="print-lista-infracoes">
        <?php foreach ($tiposInfracaoImpressao as $numItem => $descInf): ?>
            <li>
                <div class="print-checkbox" id="chk-f1-<?= $numItem ?>"></div>
                ( ) <?= $numItem ?>. <?= htmlspecialchars($descInf) ?>;
            </li>
        <?php endforeach; ?>
    </ul>
